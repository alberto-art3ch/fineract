...
public class SearchReadPlatformServiceImpl implements SearchReadPlatformService {
    ...
    public String searchSchema(final SearchConditions searchConditions) {
        ...
        final String clientMatchSql = "( (select 'CLIENT' as entityType, c.id as entityId, c.display_name as entityName, c.external_id as entityExternalId, c.account_no as entityAccountNo "
                ...
                + " from m_client c join m_office o on o.id = c.office_id where o.hierarchy like :hierarchy and (c.account_no like :search or c.display_name like UPPER(:search) or c.external_id like :search or c.mobile_no like :search) "
                + " order by c.id desc)) ";
        ...
        final String savingMatchSql = "( (select 'SAVING' as entityType, s.id as entityId, sp.name as entityName, s.external_id as entityExternalId, s.account_no as entityAccountNo "
                ...
                + " where (o.hierarchy IS NULL OR o.hierarchy like :hierarchy) and (s.account_no like :search or s.external_id like :search) order by s.id desc)) ";
        ...
        /*
         * if (searchConditions.isShareSeach()) { sql.append(shareMatchSql).append(union); }
         *
         * if (searchConditions.isGroupSearch()) { sql.append(groupMatchSql).append(union); }
         */
